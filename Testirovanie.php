...
<?php
    if($_SESSION['user']['roles'] == 'admin') {
        ...
        $stmt = $pdo->prepare('SELECT * FROM tests WHERE roles = ?');
    ...
    while($test = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ?>
                <div class="card <?=$test['category']?>">
                    <div class="container">
                        <p class="product-name"><?=$test['title']?></p>
                        <button type="button" class="btn-click" onClick="location.href='probn-test/<?=$test['probn']?>';">Пробный тест</button><br>
                        <button class="btn-click" type="submit" onClick="location.href='test/<?=$test['page']?>';" name="formBtn">Итоговый тест</button>
                    </div>
                </div>
        <?php
    }
